  ...
  <article>
    ...
    <div >
      <table>
        ...
        <form method="GET" action="struct.php" onsubmit="return checkForm()">
        <?php
          $howmuch = $_GET['howmuch'];
          $where =  $_GET['where'];
          echo '<input type="hidden" name="db" value ="'.$q_db.'">';
          echo '<input type="hidden" name="tb" value ="'.$q_tb.'">';
          echo '<input type="hidden" name="howmuch" value ="'.$howmuch.'">';
          echo '<input type="hidden" name="where" value ="'.$where.'">';

          for($i = 0; $i < $howmuch;$i++){
            echo '<tr><td>';
            echo '<input type="text" data-check-duplicate name="'.$i.'">';
            echo '</td></tr>';
          }
        ?>
      </table>
      <input type="submit" name="added" id="added" value="執行" onclick="hideButton()">
      <?php
        $check1 = 0;
        $check2 = 1;
