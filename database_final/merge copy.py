...
def process_group_by(dataF, group_by_cols):
    #print("dataF")
    #print(dataF)
    grouped_df = dataF.groupby(group_by_cols).size().reset_index(name='count')
    return grouped_df

def process_having(dataF, group_by_cols, aggregation_function_str):
    # print(group_by_cols)
    # 分組
...
#__________這裡是新的和where同層____________
        ...
        if "HAVING" in exceptOp:
            ...
            dataF = process_having(OriResult,group_by_cols,having_condition)
            # print(dataF)
            ...
            if "GROUP BY" in exceptOp:
                group_by_cols = cleanOnlySpace(exceptOp.split("GROUP BY")[1].split("HAVING")[0]).split(',')
                            # OriResult->DataFrame
                group_by_cols = [element.lower() for element in group_by_cols]#mod
                dataF = process_group_by(OriResult, group_by_cols) #mod
                json_data = dataF.to_json(orient='records')
                print(json_data)
            else:
                pass
...
def _insert_with_col(table_name: str, col_list: List[str], value_list: str) -> None:
    ...
    * 如果主鍵值已經存在，則會報錯
    ...
    if insert_data[csv_col_list[0]] in table_pk_value_list:
        print((f"PK 值: {insert_data[csv_col_list[0]]} 已經存在"))
        quit()
...
#sql_query = 'SELECT * FROM vote,students WHERE students.sid = vote.sid'

#主程式
#sql_query = 'SELECT sid,name FROM students WHERE sid > "M1105505"'
...
# NOTE 12/21 新增
#insert_with_col = "INSERT INTO students('sid', 'name', 'department', 'score') VALUES ('A11199', '王測試', '測試系', 100)"
# insert_with_col = "INSERT INTO students('sid', 'name', 'department') VALUES ('T11100', '王測試', '測試系')"
# insert_with_col = "INSERT INTO students('name', 'sid', 'score', 'department') VALUES ('王測試', 'U1100', 200, '測試系')"

# NOTE Those will raise error
# insert_with_col = "INSERT INTO students('sid', 'name', 'department', 'score') VALUES ('A11100', '王測試', '測試系', 100)"
# insert_with_col = "INSERT INTO students('name', 'department', 'score') VALUES ('王測試', '測試系', 100)"
insert_with_col = "SELECT * FROM students,classes WHERE students.sid = 1"



#sql_query = query

execute_sql(insert_with_col)