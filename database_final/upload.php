    ...
    if ($db && $table) {
        ...
        if ($uploadOk) {
            if (!file_exists($targetDir)) {
                mkdir($targetDir, 0755, true); // 創建目錄，如果不存在
            }
            // Check if the file already exists
            if (file_exists($targetFile)) {
                // Append the new data to the existing CSV file
                $newData = file_get_contents($_FILES['file']['tmp_name']);
                file_put_contents($targetFile, $newData, FILE_APPEND | LOCK_EX);
                echo 'File ' . basename($_FILES['file']['name']) . ' appended to data.csv successfully.';
            } 
            else {
            if (move_uploaded_file($_FILES['file']['tmp_name'], $targetFile)) {
                echo '檔案 ' . basename($_FILES['file']['name']) . ' 上傳成功。';
